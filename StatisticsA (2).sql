...
SET ANSI_NULLS ON
GO
CREATE OR ALTER PROCEDURE StatisticsA
...
BEGIN
	...
	DELETE	#sttcs 
	FROM	#sttcs sc
	WHERE	sc.rt IN (3, 5) -- Affected / Empty

	UPDATE	src 
	SET	src.QueryStatementId = src.QueryStatementId2
	FROM (
		SELECT	*, QueryStatementId2 = DENSE_RANK() OVER(ORDER BY QueryStatementId)
		FROM	 #sttcs
	) src
	...
	BEGIN
		...
		SELECT	QueryStatementId = sql.[SqlStatementSl].value('@StatementId', 'INT'), QueryTxt = (SELECT sql.SqlStatementSl.value('@StatementText', 'NVARCHAR(MAX)') AS '*' FOR XML PATH(N''), TYPE), 
			MissingIndexes	= sql.[SqlStatementSl].query('*:QueryPlan/*:MissingIndexes'),
			Waittypes		= sql.[SqlStatementSl].query('*:QueryPlan/*:WaitStats'),
			Warnings		= sql.[SqlStatementSl].query('*:QueryPlan/*:Warnings'),
			[Statistics]	= sql.[SqlStatementSl].query('*:QueryPlan/*:OptimizerStatsUsage') ,
			ParameterList	= sql.[SqlStatementSl].query('*:QueryPlan/*:ParameterList') 
		INTO	#plan
		FROM	@plan.nodes('*:ShowPlanXML/*:BatchSequence/*:Batch/*:Statements/*:StmtSimple') sql([SqlStatementSl])

		SELECT	Num = sttcs.rn, QueryTxt = ISNULL(pl.QueryTxt, sttcs.que), Txt = sttcs.txt, lgc.*, sttcs.QueryStatementId, MissingIndexes, Waittypes, Warnings, [Statistics], ParameterList
		INTO	##sttcslgc 
		FROM	#sttcs sttcs
